...
main :: IO ()
main = do
    assertEqual "guest can view a shared item" (Right Allowed) (runEngine (check kikanGraph MinimizeLatency requestContext (SubjectId agencyUser) view sharedItem))
    assertEqual "guest cannot view an internal item" (Right Denied) (runEngine (check kikanGraph MinimizeLatency requestContext (SubjectId agencyUser) view internalItem))
    assertEqual "guest cannot act on the shared space" (Right Denied) (runEngine (check kikanGraph MinimizeLatency requestContext (SubjectId agencyUser) act guestSpace))
    assertEqual "non-guest cannot view the project" (Right Denied) (runEngine (check kikanGraph MinimizeLatency requestContext (SubjectId bob) view sharedItem))
    assertEqual
