...
runLookup budget candidateCheck deadline graph revision token cursorState request = do
  ...
  where
    emitWindow =
      EmitWindow
        { watermark = cursorState >>= (view (#lastObject)),
        ...
        LookupTruncated
          ( encodeLookupCursor
              LookupCursorState
                { version = 2,
                  token,
                  lastObject = cursorState >>= (view (#lastObject)),
                  frontier = []
                }
          ),
      checkedAt = token
    ...
    evalRecursiveUserset allowed = do
      seeds <-
        evalRelation
          ...
          (allowed ^. #objectType)
          computedRelation
          (state & #skipRecursive ?~ recursiveStep)
      case seeds of
        Left err -> pure (Left err)
        Right seedObjects ->
...
pageLookup budget deadline (LookupLimit rawLimit) cursorState token objects = do
  ...
  let limit = max 0 rawLimit
      startAfter = cursorState >>= (view (#lastObject))
      remainingObjects =
        case startAfter of
          Nothing -> objects
      ...
      nextCursor =
        LookupCursorState
          { version = 2,
            token,
            lastObject = (view (#object)) <$> lastMaybe visible,
            frontier = []
          }
      state
  ...
  LookupCursor $
    Text.concat
      ( "lookup-v2"
          ...
          : encodeField (maybe "" (objectText . (view (#objectType))) lastObject)
          : encodeField (maybe "" (view (#objectId)) lastObject)
...
encodeFrontierEntry FrontierEntry {branchType, branchRelation, branchOrdinal, branchCursor, branchExhausted} =
  [ encodeField (objectText branchType),
    ...
    encodeField (maybe "" (view (#cursorEncoding)) branchCursor),
