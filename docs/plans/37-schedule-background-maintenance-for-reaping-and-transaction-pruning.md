...
- Decision: **(mirror entry, added by docs/plans/49 on 2026-07-09)** `reapDeletedTuplesBatchStatement` and `pruneTransactionsBatchStatement` in `en-postgres/src/En/Postgres/TupleStore.hs` now join their victim CTE back on `ctid`, not on the primary key. EP-49's EXPLAIN sweep found both hash-joining a full `Seq Scan` of the table to delete a `LIMIT`-bounded victim set (27 ms per reap batch on 250k rows; 21 ms per prune batch on a 250k-row `en_transaction`). Because `drain` in `en-server/app/Maintenance.hs` loops the batch session until it returns a short batch, a backlog of `B` rows cost `B / batchSize` full table scans — quadratic in table size, on the one code path guaranteed to meet large tables. Draining a 50,015-row backlog: 1.330 s → 0.194 s. **This plan should not re-derive the statements' plan shape**, but it *does* inherit two obligations: (a) `EN_MAINTENANCE_BATCH_SIZE` no longer trades throughput against a per-batch table scan, so the old intuition that a larger batch amortizes the scan is obsolete — pick it for lock duration instead; and (b) the integration fixture's table is too small for the planner to choose the tid path (it hash-joins on `ctid` too, harmlessly), so any performance claim about maintenance must be argued from an `EXPLAIN` against a populated database, never from the ephemeral suite.
  Rationale: The master plan `docs/masterplans/8-correct-write-path-and-storage-semantics.md` binds every batched statement over `relation_tuple` to an index-probe plan. This plan owns *when* maintenance runs; the plan shape of statements that already exist was fixed at the point the defect was measured. `ctid` is safe here because soft-deleted rows are never updated again (both writers restrict their `UPDATE` to `deleted_xid IS NULL`), `en_transaction` rows are never updated, and vacuum cannot recycle a line pointer for a tuple the statement's own snapshot can still see.
  Date: 2026-07-09
